...
public class TubeRenderer : MonoBehaviour
{
	...
	Modifications by Magnus Norrby
    */
	...
	public void SetPoints (Vector3[] points, float[,] radius, int i, int multiplier, Color color1, Color color2, int sum, int prev, int localPrev)
	{
		...
		for (int p = 0; p < points.Length; p++) {
			float f = (float)p;
			vertices [p + 1] = new TubeVertex (points [p], radius [i, (localPrev+p)/multiplier], Color.Lerp (color1, color2, (f + prev) / sum));
		}
